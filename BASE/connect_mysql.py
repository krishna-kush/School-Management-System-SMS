...
def class_search_student_for_roll_no_generator(extra):
    cr.execute(f"select id from students where class='{extra}';")
    ls = []
    for row in cr:
        # print(f"Name : {row[0]} {row[1]}\nClass : {row[2]}\nRoll_No : {row[3]}\nID : {row[4]}\nFee_Status : {row[5]}\nPending_Fee : {row[6]}\n")
        ls.append(row[0])
...
def class_search_student_for_bar_graph(extra):
    cr.execute(f"select first_name from students where class='{extra}' order by first_name;")
    ls = []
    for row in cr:
        ls.append(row[0][0])
    return ls
