...
import numpy as np, connect_mysql, idgenerator
...
def register():
    ...
    def submit():
        ...
        connect_mysql.add_student(first_name_val.get(), last_name_val.get(), clas_val.get(), id_stu, fee_status, pending_fee_val.get()) 
...
def get_info():
    ...
    def s_name_class():
        def submit():
            ...
            root = Tk()
            root.title("Result")
            extra = search_name_class.get().split()
...
def see_all_stu():
    ...
    for i in range(len(ls)//7):
        ...
        # AFTER
        listbox.insert(END, f"NAME : {ls[temp]} {ls[temp+1]}")
        listbox.insert(END, f"Class : {ls[temp+2]}")
        listbox.insert(END, f"Roll_No : {ls[temp+3]}")
        listbox.insert(END, f"ID : {ls[temp+4]}")
...
def update_stu_info():
    ...
    def up_name():
        ...
        def submit():
            extra = new_name.get().split()
            connect_mysql.update_name(extra, stu_id.get())
    ...
    def up_class():
        ...
        def submit():
            # root.destroy()
            connect_mysql.update_class(new_class.get(), stu_id.get())
    ...
    def up_fee_status():
        ...
        def submit():
            ...
            connect_mysql.update_id(fee_status.get(), pending_fee.get(), stu_id.get())
...
def delete_stu_info():
    ...
    def submit():
        ...
        connect_mysql.delete_student(stu_id.get())
...
def start():
    ...
    root.geometry(f"{widthmin+100}x{heightmin+100}")
